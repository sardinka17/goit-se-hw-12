...
def get_all_contacts(offset: int, limit: int, user: User, db: Session) -> list[Contact]:
...
def delete_contact(contact_id: int, user: User, db: Session) -> Contact | None:
    contact = db.query(Contact).filter(and_(Contact.id == contact_id, Contact.user_id == user.id)).first()
...
def get_contact_by_first_name(contact_first_name: str, user: User, db: Session) -> Contact | None:
...
def get_contact_by_last_name(contact_last_name: str, user: User, db: Session) -> Contact | None:
    return db.query(Contact).filter(
        and_(Contact.user_id == user.id,
...
def get_contact_by_email(contact_email: str, user: User, db: Session) -> Contact | None:
    return db.query(Contact).filter(
        and_(Contact.user_id == user.id,
...
def get_contacts_upcoming_birthdays(user: User, db: Session):
